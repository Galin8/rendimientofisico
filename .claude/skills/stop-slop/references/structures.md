...
| Three parallel items with identical grammar | Break the grammar on one, or cut to two |
| Any three-item list, even with varied grammar | ZeroGPT detects list density — cut to two or fold one item into the sentence body |
| Labelled parallel enumeration: "X (Chapter 1), Y (Chapter 2), Z (Chapter 3)" | AI-detectable structure. Vary the form or integrate into prose |
| Tidy closing clause on a list: "with each X shaping the others", "each building on the last" | Cut it. If the relationship matters, say it directly |
| Triple parallel subordinate clauses: "How a firm X, what tools Y, and how consumers Z" | Reads as AI list-building. Break into separate sentences with varied openings |
| Em-dash list as sentence closer: "— the X, Y and Z needed to [vague]" | Remove em-dash, cut the list or integrate into the sentence body |
